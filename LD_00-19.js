...
var LC  = ee.Image("users/jonathanreith/LD/LandCover"),
    LPP = ee.Image("users/jonathanreith/LD/ProductivityPerformance"), // 0ND/ 1D
    LPS = ee.Image("users/jonathanreith/LD/ProductivityState"),      //0ND / 1 IMP / 2D
    LPT = ee.Image("users/jonathanreith/LD/ProductivityTrend"),       //0ND / 1 IMP / 2D
    SOC = ee.Image("users/jonathanreith/LD/SOCDEG")                 //1 nodata /2 deg / 3stable /4 imp
...
Map.addLayer(LP.clip(area),{"palette":['green','PapayaWhip','Gold','Orange','red']} , 'Land Productivity');
...
var LD = LD
              .remap([1,	2,	3,	4,
                      ...
                      1,	2,	1,	0,
                      2,	2,	2,	2         ]);
Map.addLayer(LD.clip(area),{'max': 2, "palette":['green','PapayaWhip','red']} , 'Land Degradation');
//////////////////////////////////////////// EXPORT 

Export.image.toDrive({
  image: LD.clip(area),
  description: "LandDEGRADATION2020",
  scale: 30,
  region: area,
